...
def imagenet_deprocess_batch_not_detach(imgs, rescale=True):
  """
  Input:
  - imgs: FloatTensor of shape (N, C, H, W) giving preprocessed images
  ...
  for i in range(imgs.size(0)):
    ...
    img_de = img_de.mul(255).clamp(0, 255).byte()
    imgs_de.append(img_de)
  imgs_de = torch.cat(imgs_de, dim=0).float()
  imgs_de = imgs_de.requires_grad_()
  return imgs_de

def imagenet_deprocess_save_batch(imgs, rescale=True):
  """
  Only for inspection!!
  ...
  Output:
  - imgs_real: Real normal  image that can be inspected.
  ...
  img_real = imgs_de.float().detach().cpu().numpy().transpose([0, 2, 3, 1])[0].astype('uint8')
  imsave('image.png', img_real)
