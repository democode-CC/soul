...
def draw_scene_graph(objs, triples, vocab=None, specific_object=None, modified_object_pos=None, orientation='TB', **kwargs):
  """
  ...
  rankdir = orientation # {'H': 'LR', 'V': 'TD'}[orientation]
  objs_idx_list = copy.deepcopy(objs)
  if vocab is not None:
    ...
    for i in range(triples.size(0)):
      s = triples[i, 0].item()
      #print(vocab['pred_idx_to_name'], triples[i, 1].item())
  ...
  lines = [
    'digraph{',
    'graph [size="5,3",ratio="compress",bgcolor="lightgray",color="black",penwidth="3"]',
    'rankdir=%s' % rankdir,
    'nodesep="0.5"',
    'ranksep="0.5"',
  ...
  for i, obj in enumerate(objs):
    if ignore_dummies and obj == '__image__':
      continue
    if specific_object and (objs_idx_list[i] == specific_object[0]) and (i == specific_object[1]):  # Replace 'your_specific_object' with the actual object name or ID
      node_color = 'green'
    else:
      node_color = 'lightpink1'
    if i == modified_object_pos:
      node_color = 'yellow'
    lines.append(f'{i} [label="{obj}", fillcolor="{node_color}"]')
  ...
  os.system('ccomps -x %s | dot | gvpack -array3 | neato -Tpng -n2 -o %s' % (dot_filename, output_filename))
