...
from sg2i.graph import GraphTripleConv, GraphTripleConvNet
from sg2i.decoder import DecoderNetwork
from sg2i.layout import boxes_to_layout, masks_to_layout
...
import cv2
from sg2i.data import imagenet_deprocess_batch, imagenet_deprocess_batch_not_detach, imagenet_deprocess_save_batch
import torchvision.transforms as transforms

from sklearn.decomposition import PCA

import torch.distributions as tdist
from sg2i.feats_statistics import get_mean, get_std


class SG2I(nn.Module):
    """
    SG2I network. Given a source image and a scene graph, the model generates
    ...
    def __init__(self, vocab, image_size=(64, 64), embedding_dim=64,
        ...
        super(SG2I, self).__init__()

        if len(kwargs) > 0:
            print('WARNING: Model got unexpected kwargs ', kwargs)

        self.vocab = vocab
        self.image_size = image_size  # (64, 64)
        self.layout_noise_dim = layout_noise_dim  # 32

        self.feats_in_gcn = feats_in_gcn # True
        self.feats_out_gcn = feats_out_gcn # True
        self.spade_blocks = spade_blocks # False
        self.is_baseline = is_baseline # False
        self.is_supervised = is_supervised # False

        if self.is_supervised:
            self.im_to_noise_conv = nn.Conv2d(3, 32, 1, stride=1)

        self.image_feats_branch = img_feats_branch # True

        self.layout_pooling = layout_pooling # sum

        num_objs = len(vocab['object_idx_to_name']) # 179
        num_preds = len(vocab['pred_idx_to_name'])  # 46
        self.obj_embeddings = nn.Embedding(num_objs + 1, embedding_dim)  # Embedding(180, 128) node embedding
        self.pred_embeddings = nn.Embedding(num_preds, embedding_dim)  # Embedding(46, 128) edge embedding

        if self.is_baseline or self.is_supervised:
            gconv_input_dims = embedding_dim
        else:
            if self.feats_in_gcn:
                gconv_input_dims = embedding_dim + 4 + feat_dims  # 260 (128 + 4 + 128)
            else:
                gconv_input_dims = embedding_dim + 4

        if gconv_num_layers == 0:
            self.gconv = nn.Linear(gconv_input_dims, gconv_dim)
        elif gconv_num_layers > 0:  # the first layer
            gconv_kwargs = {
                'input_dim_obj': gconv_input_dims, # 260
                'input_dim_pred': embedding_dim, # 128
                'output_dim': gconv_dim, # 256
        ...
        self.gconv_net = None
        if gconv_num_layers > 1: # the rest 4 layers
            gconv_kwargs = {
                'input_dim_obj': gconv_dim, # 256
                'input_dim_pred': gconv_dim, # 256
                'hidden_dim': gconv_hidden_dim, # 512
                'pooling': gconv_pooling, # avg
                'num_layers': gconv_num_layers - 1, # 4 (5-1)
                'mlp_normalization': mlp_normalization, # none
            }
            self.gconv_net = GraphTripleConvNet(**gconv_kwargs)  # model to be unlearned

        # BB predictor model
        box_net_dim = 4
        box_net_layers = [gconv_dim, gconv_hidden_dim, box_net_dim] # list [256, 512, 4]
        self.box_net = build_mlp(box_net_layers, batch_norm=mlp_normalization) # Sequential(
    ...
    def build_obj_feats_net(self):
        ...
        if feature_model == 'vgg':
            ...
            vgg_net = T.models.vgg16(pretrained=True)  # load pretrained VGG16 model
            layers = list(vgg_net.features._modules.values())[:-1] # extract VGG16's all conv and pool, but without the last layer
            img_feats = nn.Sequential(*layers)
        
        elif feature_model == 'resnet':
            resnet = T.models.resnet50(pretrained=True)
    ...
    def forward(self, objs, triples, obj_to_img=None, boxes_gt=None, masks_gt=None, src_image=None, imgs_src=None,
        ...
        in_image = src_image.clone()  # torch.Size([32, 4, 64, 64])
        num_objs = objs.size(0) # 276
        s, p, o = triples.chunk(3, dim=1)  # All have shape (num_triples, 1)  # [369 1], [369 1], [369 1]
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (num_triples,) # [369], [369], [369]
        edges = torch.stack([s, o], dim=1)  # Shape is (num_triples, 2)  # [369, 2]

        obj_vecs = self.obj_embeddings(objs)  # (no_obj (e.g., 272), 128)
        ...
        if not (self.is_baseline or self.is_supervised):
            ...
            obj_crop = get_cropped_objs(in_image, boxes_gt, obj_to_img, feats_keep, box_keep, evaluating, mode) # torch.Size([272, 3, 16, 16])
            

            ################## VGG visual feature extraction ################
            ...
            feats_prior = high_feats * feats_keep # torch.Size([272, 128])
            ...
            if evaluating and random_feats:
                ...
                feats_prior = torch.zeros_like(feats_prior)
                boxes_prior = torch.zeros_like(boxes_prior)
            ...
            if self.feats_in_gcn:
                obj_vecs = torch.cat([obj_vecs, boxes_prior, feats_prior], dim=1)  # (128 + 4 + 128)
                # obj_vecs = torch.zeros_like(obj_vecs)
        ...
        if self.is_baseline or self.is_supervised:
            ...
            generated = torch.zeros([obj_to_img.size(0)], device=obj_to_img.device,
                                                        dtype=obj_to_img.dtype)  # (num_obj, )
            if not evaluating:
    ...
    def generate_output_features(self, objs, triples, obj_to_img=None, boxes_gt=None, masks_gt=None, src_image=None, imgs_src=None,
                keep_box_idx=None, keep_feat_idx=None, keep_image_idx=None, combine_gt_pred_box_idx=None,
                query_feats=None, mode='train', t=0, query_idx=0, random_feats=False, get_layout_boxes=False):
        """
        Required Inputs:
        ...
        #    visualize_layout(img, in_image, visualize_layout, obj_vecs, layout_boxes, layout_masks,
        #                     obj_to_img, H, W)
        obj_crop = get_cropped_objs(img, boxes_gt, obj_to_img, feats_keep, box_keep, evaluating, mode, masked_feats=False) # torch.Size([272, 3, 16, 16])

        obj_crop_feature = obj_crop.view(len(objs), -1)

        return obj_crop_feature


    def forward_visual_feats(self, objs, triples, obj_to_img=None, boxes_gt=None, masks_gt=None, src_image=None, imgs_src=None,
                keep_box_idx=None, keep_feat_idx=None, keep_image_idx=None, combine_gt_pred_box_idx=None,
                query_feats=None, mode='train', t=0, query_idx=0, random_feats=False, get_layout_boxes=False):
        """
    ...
    def generate_features(self, objs, triples, obj_to_img=None, boxes_gt=None, masks_gt=None, src_image=None, imgs_src=None,
                keep_box_idx=None, keep_feat_idx=None, keep_image_idx=None, combine_gt_pred_box_idx=None,
                query_feats=None, mode='train', t=0, query_idx=0, random_feats=False, get_layout_boxes=False, concat=True):
        '''
        for GIF/IF,
        ...
        if concat:
            ...
            return obj_vecs, boxes_prior, feats_prior  # return the three feature before concatenation
    
    def forward_sample_unlearn(self, args, objs, triples, obj_to_img=None, boxes_gt=None, masks_gt=None, ori_image=None, src_image=None, imgs_src=None,
                keep_box_idx=None, keep_feat_idx=None, keep_image_idx=None, combine_gt_pred_box_idx=None,
                query_feats=None, mode='eval', t=0, query_idx=0, random_feats=False, get_layout_boxes=False, pattern=None):
        ...
        in_image = src_image.clone()  # torch.Size([32, 4, 64, 64])
        img = ori_image.clone() # original img (i.e., ground truth)
        num_objs = objs.size(0) # 276
        s, p, o = triples.chunk(3, dim=1)  # All have shape (num_triples, 1)  # [369 1], [369 1], [369 1]
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (num_triples,) # [369], [369], [369]
        edges = torch.stack([s, o], dim=1)  # Shape is (num_triples, 2)  # [369, 2]
        ...
        if not (self.is_baseline or self.is_supervised):
            ...
            if self.feats_in_gcn:
                feats_prior = feats_prior.clone().detach()
                obj_vecs = torch.cat([obj_vecs, boxes_prior, feats_prior], dim=1)  # (128 + 4 + 128)
            else:
                obj_vecs = torch.cat([obj_vecs, boxes_prior], dim=1)
            obj_vecs = self.layer_norm(obj_vecs)
        ...
        img_pred = self.decoder_net(layout)
        ...
        obj_crop_pred = get_cropped_objs(img_pred, boxes_gt, obj_to_img, feats_keep, box_keep, evaluating, mode, masked_feats=False)


        if args.deprocess_in_loss:
            # Deprocess
            img_de = imagenet_deprocess_batch_not_detach(img) # recover processed tensor to normal image
            img_pred_de = imagenet_deprocess_batch_not_detach(img_pred)
        else:
            img_de = img
            img_de = img_de.requires_grad_()
            img_pred_de = img_pred
        
        if pattern == 'all_all':
            return img_de, img_pred_de, None, None
        elif pattern == 'del_del':
            return img_de, img_pred_de, None, None
        elif pattern == 'all_del':
            return img_de[0:1], img_pred_de[0:1], None, None
        


    def forward_feature_unlearn(self, args, objs, triples, obj_to_img=None, boxes_gt=None, masks_gt=None, ori_image=None, src_image=None, imgs_src=None,
                keep_box_idx=None, keep_feat_idx=None, keep_image_idx=None, combine_gt_pred_box_idx=None,
                query_feats=None, mode='eval', t=0, query_idx=0, random_feats=False, get_layout_boxes=False, 
                unlearn_obj=None, feature_edit=True, input_object_range='all'):
        ...
        if not (self.is_baseline or self.is_supervised):
            ...
            if feature_edit == True:
                # Feature editing
                if 'visual' in args.unlearn_feature_type:
                    feats_prior = torch.zeros_like(feats_prior)
                    # epsilon = 0.1  # A small constant value to control the noise level
                    # rand = epsilon * torch.randn_like(feats_prior)
                    # feats_prior = feats_prior + rand
                if 'obj' in args.unlearn_feature_type:
                    obj_vecs = torch.zeros_like(obj_vecs)
                if 'bbox' in args.unlearn_feature_type:
                    boxes_prior = torch.zeros_like(boxes_prior)
                if self.feats_in_gcn:
                    obj_vecs = torch.cat([obj_vecs, boxes_prior, feats_prior], dim=1)  # (128 + 4 + 128)
        ...
        # Calculate loss between recon and input
        if input_object_range == 'all':
            return img_de, img_pred_de, obj_crop, obj_crop_pred
        elif input_object_range == 'remain':
            img_remain = self.img_get_roi(img_de, boxes_gt, unlearn_obj, remain=True)
            img_pred_remain = self.img_get_roi(img_pred_de, boxes_gt, unlearn_obj, remain=True)
            obj_crop_remain = self.obj_get_roi(obj_crop, unlearn_obj, remain=True)
            obj_crop_pred_remain = self.obj_get_roi(obj_crop_pred, unlearn_obj, remain=True)
            return img_remain.unsqueeze(0), img_pred_remain.unsqueeze(0), obj_crop_remain.unsqueeze(0), obj_crop_pred_remain.unsqueeze(0)
        
        elif input_object_range == 'delete':
            img_del = self.img_get_roi(img_de, boxes_gt, unlearn_obj)
            img_pred_del = self.img_get_roi(img_pred_de, boxes_gt, unlearn_obj)
            obj_crop_del = self.obj_get_roi(obj_crop, unlearn_obj)
            obj_crop_pred_del = self.obj_get_roi(obj_crop_pred, unlearn_obj)
            return img_del.unsqueeze(0), img_pred_del.unsqueeze(0), obj_crop_del.unsqueeze(0), obj_crop_pred_del.unsqueeze(0)
    ...
    def img_get_roi(self, img, boxes, unlearn_obj, remain=False):
        margin = 2
        if remain:
            # Remain
            ...
            try:
                ...
                print("bbox wrong")
            img_roi = img[0, :, top:bottom, left:right]
            return img_roi

        
            
    
    def obj_get_roi(self, obj_crop, unlearn_obj, remain=False):
        margin = 2
        if remain:
            # Remain
            index_to_delete = unlearn_obj[1]
            indices = torch.tensor([i for i in range(obj_crop.size(0)) if i != index_to_delete]).to(obj_crop.device)
            obj_roi = torch.index_select(obj_crop, 0, indices)
            return obj_roi
        else:
            # Delete
            obj_roi = obj_crop[unlearn_obj[1]]
            return obj_roi
...
# def get_cropped_objs_no_feature_mask(imgs, boxes, obj_to_img, feats_keeps, boxes_keeps, evaluating, mode, masked_feats=True):
#     """
...
def get_cropped_objs(imgs, boxes, obj_to_img, feats_keeps, boxes_keeps, evaluating, mode, masked_feats=True):
    ...
    # cropped_objs (list N_objs (1, 3, 16, 16))
...
def tensor2image(tensor):
  ...
  image = image[0].squeeze(0)      # remove the fake batch dimension
  image = unloader(image)
  image.save('tmp_iamge.png')
