...
parser.add_argument('--specific_obj', default=20, type=int)
parser.add_argument('--data_lot_idx', default=2, type=int)
...
parser.add_argument('--random_feats', default=True, type=bool_flag, help='if true: image synthesis; if false: image reconstruction')
...
parser.add_argument('--human_objs_list', default=[3, 6, 20, 27, 48, 58, 75, 165, 170])
parser.add_argument('--random_label', default=None)
...
LOG_SAVE_PATH = RESULTS_SAVE_PATH + f'{args.specific_obj}_{args.data_lot_idx}_indirect_leakage.json'

if image_task == "image_reconstruction":
  if args.specific_obj == 6 and args.data_lot_idx == 0:
    args.random_label = 59
else:
  if args.specific_obj == 20 and args.data_lot_idx == 2:
    args.random_label = 62
...
def modify_obj(args, object_label, triplet, unlearned_object, leak_mode=None):
    # Extract the label and position of the unlearned object
    unlearn_label, unlearn_pos = unlearned_object

    # Initialize modified labels as a copy of the original labels to avoid in-place changes
    modified_labels = object_label.clone()

    # Build a graph from the triplets to check connections
    G = nx.Graph()
    for subj, pred, obj in triplet.cpu().numpy():  # Move to CPU for compatibility with networkx
        if pred != 0:
          G.add_edge(subj, obj)
    
    # Ensure all nodes in object_label are added to the graph, including isolated nodes
    for i in range(len(object_label)):
        if i not in G:
            G.add_node(i)

    if leak_mode == "fuzzy":
        # 1. Fuzzy Query Attack: replace the unlearned object's label with a general/fuzzy label, e.g., 0
        modified_labels[unlearn_pos] = 6  # Assuming 0 is the fuzzy/general label
        modified_pos = unlearn_pos

    elif leak_mode == "neighbor":
        # 2. Neighbor Query Attack: find a neighboring object directly connected in the triplet graph
        if unlearn_pos in G:
            neighbors = list(G.neighbors(unlearn_pos))
        else:
            neighbors = []

        # If there are no neighbors, terminate early
        if not neighbors:
            print("No neighboring objects found. Terminating early.")
            exit()  # or exit() to terminate the whole program if required

        # Choose the first neighbor to modify
        neighbor_pos = neighbors[0]
        random_label = args.random_label
        modified_labels[neighbor_pos] = random_label  # Replace with general label (0) or another choice
        modified_pos = neighbor_pos

    elif leak_mode == "singleton":
        # 3. Singleton Query Attack: find an object not connected to the unlearned object

        # Find all nodes that are not connected to the unlearned object
        disconnected_nodes = [i for i in G.nodes if i != unlearn_pos and not nx.has_path(G, unlearn_pos, i)]

        # If no such nodes exist, terminate early
        if not disconnected_nodes:
            print("No disconnected singleton objects found. Terminating early.")
            exit()  # or return to terminate only this function
        
        # If there are disconnected nodes, choose one randomly to modify
        disconnected_nodes = [i for i in disconnected_nodes if i !=8]
        singleton_pos = disconnected_nodes[0]
        random_label = args.random_label
        modified_labels[singleton_pos] = random_label  # Replace with a random label from human_objs_list
        modified_pos = singleton_pos

    return modified_labels, modified_pos
...
def main(args):
    ...
    persp = ['a1', 'a2', 'a3']
    leakages = ['fuzzy', 'neighbor', 'singleton']
    leakages_with_original = ['fuzzy', 'neighbor', 'singleton', 'original']
    ...
    all_unlearned_model = {method: {'model': unlearned_model, 
                                    'metrics': {leakage: copy.deepcopy(all_metrics) for leakage in leakages_with_original}, 
                                    'output':{leakage: None for leakage in leakages_with_original},  
                                    'img_pred': {leakage: None for leakage in leakages_with_original}, 
                                    'roi': {leakage: None for leakage in leakages_with_original}, 
                                    'resize_roi': {leakage: None for leakage in leakages_with_original}, 
                                    'norm_resize_roi':  {leakage: None for leakage in leakages_with_original}, 
                                    'non_roi':  {leakage: None for leakage in leakages_with_original}, 
                                    'norm_non_roi':  {leakage: None for leakage in leakages_with_original}} for method, unlearned_model in all_unlearned_model.items()}
    ...
    for i in range(len(sample_list)): # For all samples in the batch
      ...
      ###################### modify scene graph ######################
      modified_pos_in_different_leak = {'fuzzy':None, 'neighbor':None, 'singleton':None} 
      modified_objs_list = {'fuzzy':None, 'neighbor':None, 'singleton':None} 
      for leak in leakages:
           objs_modified, modified_pos = modify_obj(args, objs, triples, args.unlearn_obj_and_idx, leak_mode=leak)
           modified_objs_list[leak] = objs_modified
           modified_pos_in_different_leak[leak] = modified_pos



      ###################### obtain unlearned models' outputs ######################
      for method in all_unlearn_method:
        all_unlearned_model[method]['output']['original'] = all_unlearned_model[method]['model'](objs, triples, obj_to_img, boxes_gt=boxes, masks_gt=masks, src_image=imgs_in,
                                    keep_box_idx=torch.ones_like(dropimage_indices), keep_feat_idx=dropfeats_indices,
                                    keep_image_idx=dropimage_indices, random_feats=args.random_feats, mode='eval')[0]
        ...
        all_unlearned_model[method]['img_pred']['original'] = imagenet_deprocess_batch(all_unlearned_model[method]['output']['original']).float()
        all_imgs_to_demo.append(all_unlearned_model[method]['img_pred']['original'])

        for leak in leakages:
           all_unlearned_model[method]['output'][leak] = all_unlearned_model[method]['model'](objs_modified, triples, obj_to_img, boxes_gt=boxes, masks_gt=masks, src_image=imgs_in,
                                    keep_box_idx=torch.ones_like(dropimage_indices), keep_feat_idx=dropfeats_indices,
                                    keep_image_idx=dropimage_indices, random_feats=args.random_feats, mode='eval')[0]
           all_unlearned_model[method]['img_pred'][leak] = imagenet_deprocess_batch(all_unlearned_model[method]['output'][leak]).float()
      ...
      if args.save_images:
        ...
        for method in args.methods_involved_list:
           for leakage in leakages:
            save_images(args, candidate_, 
                      all_imgs_to_demo,
                      img_idx, boxes, box_on=box_on, all_data=f_unlearning_list, indirect_mode=leakage, indirect_img=all_unlearned_model[method]['img_pred'][leak])
        
        for leakage in leakages:
           visualize_scene_graphs(args, obj_to_img, modified_objs_list[leak], triples, vocab, args.device, img_idx=i, indirect_mode=leakage, modified_obj_pos=modified_pos_in_different_leak[leakage])
      ...
      if i == args.candidate:
        ...
        for method in all_unlearn_method:
          for leak in leakages_with_original:
             
            all_unlearned_model[method]['roi'][leak] = model.img_get_roi(all_unlearned_model[method]['img_pred'][leak], boxes, unlearn_obj).unsqueeze(0)
        ...
        for method in all_unlearn_method:
          for leak in leakages_with_original:
            all_unlearned_model[method]['resize_roi'][leak] = resize_transform(all_unlearned_model[method]['roi'][leak])

        ## A1_SSIM
        for method in all_unlearn_method:
          for leak in leakages_with_original:
            all_unlearned_model[method]['metrics'][leak]['a1']['ssim'] = ssim(
                          (resized_img_pred_roi[0:1, :, :, :] / 255.0).cpu().numpy().squeeze(0).transpose(1, 2, 0),
                          (all_unlearned_model[method]['resize_roi'][leak][0:1, :, :, :] / 255.0).cpu().numpy().squeeze(0).transpose(1, 2, 0),
                          win_size=3,
        ...
        for method in all_unlearn_method:
          for leak in leakages_with_original:
            all_unlearned_model[method]['norm_resize_roi'][leak] = all_unlearned_model[method]['resize_roi'][leak][0:1, :, :, :] / 127.5 - 1
            all_unlearned_model[method]['metrics'][leak]['a1']['lpips'] = float(lpips_model.forward(all_unlearned_model[method]['norm_resize_roi'][leak], normed_resized_img_pred_roi).detach().cpu().numpy())

        ## A1_MAE
        for method in all_unlearn_method:
          for leak in leakages_with_original:
            all_unlearned_model[method]['metrics'][leak]['a1']['mae'] = torch.mean(torch.abs(all_unlearned_model[method]['roi'][leak] - img_pred_roi)).cpu().item()

        ## A1_Changed Pixel (cp)
        
        for method in all_unlearn_method:
          for leak in leakages_with_original:
            all_unlearned_model[method]['metrics'][leak]['a1']['cp'] = count_changed_pixels(all_unlearned_model[method]['roi'][leak], img_pred_roi) / obj_roi_pixel_num
      
    #   ################## Assessment 2: Rest part of the target sample ##################
    #   if i == args.candidate:
    #     img_pred_non_roi = model.img_get_roi(ori_img_pred, boxes, unlearn_obj, remain=True)
    #     for method in all_unlearn_method:
    #       all_unlearned_model[method]['non_roi'] = model.img_get_roi(all_unlearned_model[method]['img_pred']['original'], boxes, unlearn_obj, remain=True)
    ...
    #     normed_resized_img_pred_roi = resized_img_pred_roi[0:1, :, :, :] / 127.5 - 1
    #     for method in all_unlearn_method:
    #       all_unlearned_model[method]['roi'] = model.img_get_roi(all_unlearned_model[method]['img_pred']['original'], boxes, same_obj).unsqueeze(0)
