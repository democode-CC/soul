...
def argument_parser():
  ...
  parser.add_argument('--negative_guidance_weight', default=1.0, type=float)
...
def main(args):
  ...
  args.checkpoint_start_from = 'spade_64_vg_model.pt' 
    ...
    for batch_idx, batch in enumerate(tqdm.tqdm(train_loader)): 
      ...
      if obj_discriminator is not None:
        ...
        if batch_idx == 0:
          ...
          specific_obj_indices = (objs_ == args.specific_obj).nonzero(as_tuple=True)[0] 
          if specific_obj_indices.nelement() > 0:
              imgs_roi = model.img_get_roi(imgs, boxes, unlearn_obj).unsqueeze(0)
              imgs_pred_roi = model.img_get_roi(imgs_pred, boxes, unlearn_obj).unsqueeze(0)
              negative_guidance_loss = F.l1_loss(imgs_roi, imgs_pred_roi)
              negative_guidance_loss = -negative_guidance_loss
              total_loss = add_loss(total_loss, negative_guidance_loss, losses, 'negative_guidance_loss', args.negative_guidance_weight)
          

          if specific_obj_indices.nelement() > 0:
            mask = objs_ != args.specific_obj
            objs_ = objs_[mask]
            boxes_ = boxes_[mask]
            obj_to_img_ = obj_to_img_[mask]
        #####################################################################
      ...
      if t % args.num_iterations == 0:
        ...
        checkpoint_path_latest = os.path.join(args.output_dir,
                              f'{args.checkpoint_name}_model_ft_obj_{args.specific_obj}_{args.data_lot_idx}_obj_ng.pt')
