...
def main(args):
  ...
  args.checkpoint_start_from = 'spade_64_vg_model.pt'  # path pre-trained (original) model
  ...
  while True:
    ...
    for batch_idx, batch in enumerate(tqdm.tqdm(train_loader)): 
      ...
      ############################ negative guidance ############################
      if batch_idx == 0: # when it is the delete sample
        skip_pixel_loss = (model_boxes is None)
        negative_guidance_loss, losses = calculate_model_losses(
                              args, skip_pixel_loss, imgs, imgs_pred,
                              boxes, boxes_pred)
        
        total_loss = -negative_guidance_loss
      ...
      if t % args.num_iterations == 0:
        ...
        checkpoint_path_latest = os.path.join(args.output_dir,
                              f'{args.checkpoint_name}_model_ft_obj_{args.specific_obj}_{args.data_lot_idx}_sample_ng.pt')
